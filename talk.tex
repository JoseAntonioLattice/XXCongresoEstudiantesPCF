...
\begin{tikzpicture}
	...
	\draw[visible on = <1>] (6,1) node[]{Fijamos $\beta = 0.25$.};
	...
	\draw[visible on = <6>] (4,-2) node[]{$\Delta S = 2(0.25)(-1)(-1-1-1+1) = 1$};
...
$$A_{\mu}(x) \in \mathfrak{su}(N) \to U_{\mu}(x) \in \text{SU}(N), \ \ \ U_{\mu}(x) = e^{iA_{\mu}(x)}$$ 
	...
	\begin{tikzpicture}[scale = 0.7]
		...
		\draw[] (2,0) node[below right] {\small  $x+\hat\mu$};
		\draw[] (0,2) node[above left] {\small  $x+\hat\nu$};
		\draw[] (2,2) node[above right] {\small  $x+\hat\mu+\hat\nu$};
